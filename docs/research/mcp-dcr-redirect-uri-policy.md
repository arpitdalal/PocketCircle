...
PocketCircle MCP Worker DCR (`clientRegistrationCallback` → `evaluateClientRegistrationPolicy`)
needed a redirect policy that covers Cursor, VS Code, Claude Code, MCP Inspector, and
similar native clients without a Cursor-only host hardcode — or a principled configurable
allowlist if pure generality is unsafe.

Current policy: `packages/mcp-worker/src/client-registration-policy.ts` — https +
loopback http always, plus schemes from `MCP_DCR_ALLOWED_SCHEMES` (Wrangler default
`cursor,vscode`) with a hard dangerous-scheme denylist. DCR is rate-limited
(`MCP_DCR_RATE_LIMITER`, #354); CIMD preferred. Users paste the MCP resource URL
into clients (`docs/research/mcp-connections-empty-state-patterns.md`).

This note captured options before ship; **option C is what shipped** (see
Recommendation).
...
| **PocketCircle (shipped)** | https ∨ loopback http ∨ schemes in `MCP_DCR_ALLOWED_SCHEMES` (default `cursor,vscode`). Hard denylist. Cap 20 URIs. | `packages/mcp-worker/src/client-registration-policy.ts`; `wrangler.jsonc` vars |
...
| **VS Code** | `http://127.0.0.1:33418`, `https://vscode.dev/redirect` (+ insiders / ephemeral port). | Loopback + claimed https; `vscode://` may appear on some installs. | [MCP developer guide](https://code.visualstudio.com/api/extension-guides/ai/mcp); [vscode#278512](https://github.com/microsoft/vscode/issues/278512) |
| **Claude Code** | `http://localhost:PORT/callback` (random or `--callback-port`). Paste callback URL when loopback unreachable. | Loopback-first; no private-use scheme required for documented flow. | [code.claude.com/docs/en/mcp](https://code.claude.com/docs/en/mcp) |
| **MCP Inspector** | Origin + `/oauth/callback` and `/oauth/callback/debug`. | Same-origin HTTPS or loopback depending on how Inspector is hosted. | [inspector#930](https://github.com/modelcontextprotocol/inspector/issues/930) |
| **CIMD example (MCP)** | `http://127.0.0.1:3000/callback`, `http://localhost:3000/callback` | Spec example is loopback http only. | [2026-07-28 client-registration](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization/client-registration) |
...
- **Config:** None.
- **Fit:** Strict MCP MUST baseline; use as empty-allowlist mode (`MCP_DCR_ALLOWED_SCHEMES=`).
...
### C. HTTPS + loopback + configurable scheme allowlist (env / Wrangler) — **shipped**

- **Rule:** A as base. Plus schemes listed in config (e.g. `cursor`, `vscode`). Hard denylist of dangerous schemes never overridable (http_mcp pattern).
- **Pros:** No Cursor hardcode in source. Ops add schemes when needed. Matches Laravel + http_mcp industry pattern.
- **Cons:** Must maintain list; short schemes remain spoofable (RFC 8252 acknowledges this).
- **Config:** `MCP_DCR_ALLOWED_SCHEMES=cursor,vscode` (comma-separated, no `:`). Wrangler `vars` default; `.dev.vars` / vitest override as needed. Empty string = https + loopback only.
- **Fit:** **Shipped.** Prod and local examples default to `cursor,vscode`.
...
- **Config:** Would be a separate var (not shipped); e.g. prefix list — prefer C unless scheme-wide feels too wide.
- **Fit:** Optional future tightening; not implemented.

### E. Cursor-hardcode — **reject**

- **Rule:** Special-case `cursor://anysphere.cursor-mcp/…` in application code (pre-ship interim).
- **Pros:** Unblocks known Cursor fallback with one if.
- **Cons:** Product rejected. Does not scale. Encodes Anysphere into PocketCircle policy.
- **Config:** Code constant.
- **Fit:** **Rejected / removed.** Prefer C.
...
**Shipped (2026-09):** option **C** — https + loopback always; `MCP_DCR_ALLOWED_SCHEMES`
defaults to `cursor,vscode` in `wrangler.jsonc` / `.dev.vars.example`. Hard denylist in
code. No Cursor-host hardcode (E rejected).

Historical research ranking (pre-ship): A as strict default, C as empty escape hatch.
Interop reality (Cursor still registers `cursor://` in the same DCR set as loopback)
made a non-empty C default the right prod choice. B alone still fails Cursor. Blind
“any non-http” rejected (fastmail-mcp-remote review).
