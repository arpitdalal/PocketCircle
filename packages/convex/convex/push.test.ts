...
describe("Push mirror from Notification Center", () => {
  ...
  it("skips subscriptions lastSeen before PUSH_DELIVERY_SINCE_MS", async () => {
    ...
    expect(isSubscriptionEligibleForPushDelivery({ lastSeenAt: floor - 1, pushSwVersion: 2 })).toBe(
      false,
    );
    expect(isSubscriptionEligibleForPushDelivery({ lastSeenAt: floor, pushSwVersion: 2 })).toBe(
      true,
    );
    ...
    expect(
      isSubscriptionEligibleForPushDelivery({ lastSeenAt: Date.now(), pushSwVersion: 2 }),
    ).toBe(true);
    vi.stubEnv("PUSH_DELIVERY_SINCE_MS", String(floor));
    const t = convexTest(schema, modules);
    registerPushWorkpool(t);
